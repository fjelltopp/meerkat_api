...
class MeerkatAPITestCase(unittest.TestCase):
    ...
    def test_export_data(self):
        ...
        rv = self.app.get('/export/data', headers={**{"Accept": "text/csv"}, **headers=settings.header})
        self.assertEqual(rv.status_code, 200)
    ...
    def test_export_category(self):
        ...
        rv = self.app.get('/export/category/cd_tab/cd?variables=[["icd_code", "icd code"], ["icd_name$cd_tab", "Name"], ["alert_link$alert_investigation$alert_labs./return_lab", "Return Labs"], ["clinic", "Clinic"], ["meta/instanceID", "uuid"], ["end$month", "Month"], ["end$year", "Year"], ["end$epi_week", "epi_week"]]', headers={**{"Accept": "text/csv"}, **headers=settings.header})
        self.assertEqual(rv.status_code, 200)
        lines = rv.data.decode("utf-8").strip().split("\r\n")
    ...
    def test_export_forms(self):
        ...
        rv = self.app.get('/export/form/case', headers={**{"Accept": "text/csv"}, **headers=settings.header})
        self.assertEqual(rv.status_code, 200)
        lines = rv.data.decode("utf-8").strip().split("\r\n")
        ...
        rv = self.app.get('/export/form/case?fields=icd_code,intro./module', headers={**{"Accept": "text/csv"}, **headers=settings.header})
        self.assertEqual(rv.status_code, 200)
        lines = rv.data.decode("utf-8").strip().split("\r\n")
        self.assertEqual(len(lines), 11)
        for line in c:
    ...
    def test_export_alerts(self):
        ...
        rv = self.app.get('/export/alerts', headers={**{"Accept": "text/csv"}, **headers=settings.header})
        self.assertEqual(rv.status_code, 200)
        lines = rv.data.decode("utf-8").strip().split("\r\n")
